...
from topic4_parent_model_metrics.price_hist_and_stats_incremental_mixed_cond.util_parent_mdl_price_hist_and_stats_incremental_mixed_cond import (    
    weekly_aggregation_by_parent_mdl       
)
...
# Instantiate a DAG
@dag(
    'parent_mdl_price_hist_and_stats_incremental_mixed',    
    default_args=default_args,
    description='A DAG to process incrementally parent model mixed condition (New, and Pre-Owned) metrics',    
    # schedule='0 0 * * 3', # Run weekly on Wednesday at midnight
    schedule=None,
    catchup=False,
    max_active_runs=1,    
    max_consecutive_failed_dag_runs=1, # If DAG fails once it gets paused
    tags=['metrics','price_history', 'parent_model', 'mixed_condition']
)

def parent_mdl_price_hist_and_stats_incremental_mixed_dag():
    # Define constants
    brands_list = brands_ready_for_metrics()
    chunk_size = 3 # Adjust as needed
    list_of_chunks = split_brands_list(brands_list, chunk_size)
    condition = 'mixed'
    metrics_config_key = 'parent_mdl_metrics_config_key'
    prod_schema_name = 'metrics'
    prod_table_1_name = 'wcc_parent_mdl_wkl_price_and_stats_mixed' 
    description = 'Parent Model Weekly Price and Stats Mixed Condition Data'

    # Task 1: Load new listings
    @task(
        task_id='load_latest_week_listings',
        pool='parent_model_metrics_pool',
        retry_delay=timedelta(minutes=5),
        retries=1,
        depends_on_past=True,  
    ...
    def task_load_latest_week_listings(chunk: list[str], chunk_identifier: str, metrics_config_key: str, **context) -> str:
        try:
            ...
            # Query data from wcc_parent_mdl_wkl_price_and_stats_mixed rds table directly related to the brands that are part of the chunk.
    ...
    # Task 2: weekly_aggregation_by_parent_mdl
    @task(task_id='weekly_aggregation_by_parent_mdl',
          pool='parent_model_metrics_pool',
          doc_md="""
    ...
    def task_weekly_aggregation_by_parent_mdl(s3_path, chunk_identifier, **context):
        df = load_df_from_s3_airflow_temp(s3_path)
        wkl_df = weekly_aggregation_by_parent_mdl(df)
    ...
    @task(task_id='combine_new_and_historical_data',
          ...
          Combines the new and historical data for the parent model. 
          """
    )
    ...
    @task(task_id='extend_and_forward_fill',
          ...
          Extends the data to the latest date in the dataframe and forward fills missing values
            for each brand and parent model combination.
          """
    )
    def task_extend_and_forward_fill(s3_path, chunk_identifier, metrics_config_key, **context):
        df = load_df_from_s3_airflow_temp(s3_path)
        extended_and_ffilled = apply_extend_and_forward_fill(combined_data=df, metrics_config_key=metrics_config_key)
    ...
    for chunk in list_of_chunks:
        ...
        with TaskGroup(group_id=group_id) as chunk_group:
            ...
            weekly_agg_path = task_weekly_aggregation_by_parent_mdl(latest_week_listings_path, chunk_identifier)
            ...
            # Task 7: Upload final data to RDS
            task_generic_upload_to_rds(dollar_and_pct_change_path, prod_schema_name, prod_table_1_name, description)
    ...
    trigger_next_dag = TriggerDagRunOperator(
        task_id='trigger_brand_price_hist_and_stats_incremental_mixed_dag',
        trigger_dag_id='brand_price_hist_and_stats_incremental_mixed',
        wait_for_completion=False,
    )
    # Set dependencies between groups
    chunk_groups >> cleanup_group >> trigger_next_dag

dag = parent_mdl_price_hist_and_stats_incremental_mixed_dag()
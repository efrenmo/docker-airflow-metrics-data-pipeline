...
from topic5_brand_metrics.price_hist_and_stats_incremental_mixed_cond.util_brand_price_hist_and_stats_incremental_mixed_cond import (    
    weekly_aggregation_by_brand      
)
...
# Instantiate a DAG
@dag(
    'brand_price_hist_and_stats_incremental_mixed',    
    default_args=default_args,
    description='A DAG to process incrementally brand mixed condition (New, and Pre-Owned) metrics',    
    # schedule='0 0 * * 3', # Run weekly on Wednesday at midnight
    schedule=None,
    catchup=False,
    max_active_runs=1,    
    max_consecutive_failed_dag_runs=1, # If DAG fails once it gets paused
    tags=['metrics','price_history', 'brand', 'mixed_condition']
)

def brand_price_hist_and_stats_incremental_mixed_dag():
    # Define constants
    brands_list = brands_ready_for_metrics()
    chunk_size = 3 # Adjust as needed
    list_of_chunks = split_brands_list(brands_list, chunk_size)
    condition = 'mixed'
    metrics_config_key = 'brand_metrics_config_key'
    # prod_schema_name = 'metrics'
    # prod_table_1_name = 'wcc_brand_wkl_price_and_stats_mixed' 
    # description = 'Brand Weekly Price and Stats Mixed Condition Data'

    # Task 1: Load new listings
    @task(
        task_id='load_latest_week_listings',
        pool='brand_metrics_pool',
        retry_delay=timedelta(minutes=5),
        retries=1,
        depends_on_past=True,  
    ...
    def task_load_latest_week_listings(chunk: list[str], chunk_identifier: str, metrics_config_key: str, **context) -> str:
        try:
            ...
            # Query data from public.metrics_wcc_data_enriched_rm_outl rds table for the brands that are part of the chunk.
    ...
    # Task 2: weekly_aggregation_by_brand
    @task(task_id='weekly_aggregation_by_brand',
          pool='brand_metrics_pool',
          doc_md="""
    ...
    def task_weekly_aggregation_by_brand(s3_path, chunk_identifier, **context):
        df = load_df_from_s3_airflow_temp(s3_path)
        wkl_df = weekly_aggregation_by_brand(df)
    ...
    @task(task_id='combine_new_and_historical_data',
          ...
          Combines the new and historical data for the brand. 
          """
    )
    ...
    for chunk in list_of_chunks:
        ...
        with TaskGroup(group_id=group_id) as chunk_group:
            ...
            weekly_agg_path = task_weekly_aggregation_by_brand(latest_week_listings_path, chunk_identifier)
    ...
    trigger_next_dag = TriggerDagRunOperator(
        task_id='trigger_market_makeup_pipeline_dag',
        trigger_dag_id='market_makeup_pipeline',
        wait_for_completion=False,
    )    
    # Set dependencies between groups
    chunk_groups >> cleanup_group >> trigger_next_dag

dag = brand_price_hist_and_stats_incremental_mixed_dag()